...
const prepareSpreadData = (algorithmResults) => {
  ...
  const datasets = Object.entries(algorithmResults).map(([algorithm, algorithmData]) => {
    ...
    const data = [0, ...sortedSeedSizes.map(size => spreadBySeedSize[size] || null)];
  ...
  return {
    labels: ["0 nodes", ...sortedSeedSizes.map(size => `${size} nodes`)],
    datasets
  };
};
...
const InfluenceSpreadChart = ({ algorithmResults }) => {
  ...
  const chartOptions = {
    ...
    plugins: {
      ...
      tooltip: {
        callbacks: {
          title: (tooltipItems) => {
            return tooltipItems[0].label === '0 nodes' 
              ? 'No seeds selected' 
          ...
          label: (context) => {
            ...
            return `${label}: ${value} nodes activated`;
          }
        }
      }
    },
  ...
  return (
    <div className="influence-spread-chart">
      <Card className="chart-card">
